...
import { findLatestCompletedWeek } from "./weekly-report.js";
import {
...
main().catch((error) => {
  console.error("Unable to preview the division rivalry tracker.");
  console.error(error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  console.log(
    `Replaying ${league?.name ?? "league"} ${league?.season ?? ""} interdivision series through Week ${latestCompletedWeek}\n`
  );

  const currentDivisionNames = getDivisionNames(league);

  let lastReport = null;
  for (const quarterWeek of RIVALRY_QUARTER_WEEKS) {
    if (quarterWeek > latestCompletedWeek) {
      break;
    }

    const allTimeSeries = await buildAllTimeDivisionSeries({
      league,
      fetchJson,
      regularSeasonEndWeek: REGULAR_SEASON_END_WEEK,
      currentThroughWeek: quarterWeek,
      currentDivisionNames,
    });

    const report = buildDivisionRivalryReport({
      league,
      rosters,
      users,
      matchupsByWeek,
      throughWeek: quarterWeek,
      allTimeSeries,
    });

    if (!report) {
      console.log(`Week ${quarterWeek}: no interdivision games played yet.\n`);
      continue;
    }
  ...
  // No quarter boundary reached yet (e.g. previewing the live, in-progress
  // season) — fall back to showing the series through whatever's completed
  // so far, purely for preview purposes (the live bot would never send this).
  if (!lastReport && latestCompletedWeek >= 1) {
    const fallbackAllTimeSeries = await buildAllTimeDivisionSeries({
      league,
      fetchJson,
      regularSeasonEndWeek: REGULAR_SEASON_END_WEEK,
    ...
    const fallbackReport = buildDivisionRivalryReport({
      league,
      rosters,
      users,
      matchupsByWeek,
      throughWeek: latestCompletedWeek,
      allTimeSeries: fallbackAllTimeSeries,
    });

    if (fallbackReport) {
      console.log(
        `(No quarter boundary reached yet — showing the series through the latest completed week, Week ${latestCompletedWeek}, for preview purposes only)\n`
      );
      console.log(fallbackReport.textMessage);
      console.log("");
  ...
  if (!lastReport) {
    console.log("No interdivision games to preview yet.");
    return;
  }
  ...
  if (args.send) {
    ...
    await sendPreviewToChat({
      chatId: targetChatId,
      message: lastReport.textMessage,
    });
  }
}
...
async function sendPreviewToChat({ chatId, message }) {
  ...
  try {
    console.log("");
    console.log(`Sending preview division rivalry message to test chat ${chatId}`);
    await startSnapchatSession(bot);
    await sendMessageWithRetry(bot, chatId, message);
    console.log("Preview division rivalry message sent.");
  } finally {
    if (bot.browser) {
