...
main().catch((error) => {
  console.error("Unable to preview the playoff bracket/recap.");
  console.error(error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  const [rosters, users, regularSeasonMatchupsByWeek] = await Promise.all([
  ...
  if (!isBracketTrustworthy(latestCompletedRegularSeasonWeek, REGULAR_SEASON_END_WEEK)) {
    console.log(
      "Regular season isn't complete yet for this league, so winners_bracket would just be a " +
        "meaningless placeholder. Try `--previous` to preview against a finished season, or " +
        "`--league-id <id>` for a specific one."
    );
    return;
  }

  const winnersBracket = await fetchJson(
  ...
  if (type === "reveal" || type === "both") {
    console.log("=== Playoff Bracket Reveal ===\n");
    // Replaying a *finished* season means winners_bracket already has every
    // result filled in -- strip results back to the genuinely-undecided
    // shape Sleeper actually returns right after Week 14, so this preview
    // matches what the league would really have seen, not a result-spoiled
    // version of it.
    const freshBracket = stripDecidedResults(winnersBracket);
    const reveal = buildPlayoffBracketReveal({
      league,
      standings: report.standings,
      winnersBracket: freshBracket,
      playoffWeekStart,
    });

    if (!reveal) {
      console.log("No bracket reveal to show (winners_bracket data unavailable).\n");
    } else {
      console.log(reveal.textMessage);
      console.log("");
      if (args.send) {
        await sendPreviewToChat({ chatId: resolveTargetChatId(), message: reveal.textMessage });
      }
    }
  }

  if (type === "recap" || type === "both") {
    if (!isWeekScored(playoffMatchupsByWeek, lastPlayoffWeek)) {
      console.log(`Championship week (${lastPlayoffWeek}) isn't scored yet for this league.\n`);
      return;
    }

    console.log("=== Championship Recap ===\n");
    ...
    console.log(championshipRecap?.textMessage ?? "null");
    console.log("");
    ...
    console.log(seasonRecap?.textMessage ?? "null");
    console.log("");

    if (args.send) {
      if (championshipRecap) {
        await sendPreviewToChat({ chatId: resolveTargetChatId(), message: championshipRecap.textMessage });
      }
      if (seasonRecap) {
        await sendPreviewToChat({ chatId: resolveTargetChatId(), message: seasonRecap.textMessage });
      }
    }
  }
}

function stripDecidedResults(winnersBracket) {
  return (winnersBracket ?? []).map((entry) => {
    const next = { ...entry, w: null, l: null };
    for (const side of ["t1", "t2"]) {
      if (next[`${side}_from`] != null) {
        next[side] = null;
      }
    }
    return next;
...
async function sendPreviewToChat({ chatId, message }) {
  ...
  try {
    console.log("");
    console.log(`Sending preview playoff message to test chat ${chatId}`);
    await startSnapchatSession(bot);
    await sendMessageWithRetry(bot, chatId, message);
    console.log("Preview playoff message sent.");
  } finally {
    if (bot.browser) {
