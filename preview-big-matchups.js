...
import { BIG_MATCHUPS_MIN_WEEK, buildBigMatchupsReport } from "./big-matchups.js";
...
main().catch((error) => {
  console.error("Unable to preview the big matchups report.");
  console.error(error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  const weeksToPreview = args.week
    ...
    : rangeInclusive(BIG_MATCHUPS_MIN_WEEK + 1, Math.min(latestCompletedWeek + 1, REGULAR_SEASON_END_WEEK));

  if (weeksToPreview.length === 0 || weeksToPreview[0] < 1) {
    throw new Error(
      "No previewable weeks found. Try `--previous` for a completed season, or `--week N` directly."
    );
  }

  console.log(
    `Replaying ${league?.name ?? "league"} ${league?.season ?? ""} big matchups, Weeks ${weeksToPreview[0]}-${weeksToPreview[weeksToPreview.length - 1]}\n`
  );

  let lastReport = null;
  for (const displayWeek of weeksToPreview) {
    const throughWeek = displayWeek - 1;
    if (throughWeek < 1) {
      continue;
    }
    ...
    if (!bigMatchupsReport) {
      console.log(`Week ${displayWeek}: no marquee matchups this week.\n`);
      continue;
    }

    console.log(bigMatchupsReport.textMessage);
    console.log("");
    lastReport = bigMatchupsReport;
  }

  if (!lastReport) {
    console.log("No big matchups report to preview yet.");
    return;
  }
...
async function sendPreviewToChat({ chatId, message }) {
  ...
  try {
    console.log("");
    console.log(`Sending preview big matchups message to test chat ${chatId}`);
    await startSnapchatSession(bot);
    await sendMessageWithRetry(bot, chatId, message);
    console.log("Preview big matchups message sent.");
  } finally {
    if (bot.browser) {
