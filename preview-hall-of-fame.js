...
const HALL_OF_FAME_FILE = path.join(__dirname, ".state", "hall-of-fame.json");
...
main().catch((error) => {
  console.error("Unable to preview the Hall of Fame.");
  console.error(error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  let hallOfFame;
  if (args.fromCache) {
    hallOfFame = await loadCachedHallOfFame();
    if (!hallOfFame.seededFromHistory) {
      console.log(
        "(.state/hall-of-fame.json hasn't been seeded yet -- run without --from-cache for a fresh chain walk.)\n"
      );
    }
  } else {
    console.log("Walking the full previous_league_id chain (this can take a moment)...\n");
...
async function loadCachedHallOfFame() {
  try {
...
async function sendPreviewToChat({ chatId, message }) {
  ...
  try {
    console.log("");
    console.log(`Sending preview Hall of Fame message to test chat ${chatId}`);
    await startSnapchatSession(bot);
    await sendMessageWithRetry(bot, chatId, message);
    console.log("Preview Hall of Fame message sent.");
  } finally {
    if (bot.browser) {
