...
import { buildWeeklyPlayoffReport, isBracketTrustworthy } from "./playoffs.js";
...
main().catch((error) => {
  console.error("Unable to preview the weekly playoff report.");
  console.error(error);
  process.exitCode = 1;
});
...
async function main() {
  ...
  const weeksToPreview = args.week
    ? [clampWeek(Number(args.week), playoffWeekStart, lastPlayoffWeek)]
    : [playoffWeekStart, playoffWeekStart + 1, lastPlayoffWeek];

  console.log(
    `Replaying ${league?.name ?? "league"} ${league?.season ?? ""} playoffs, Weeks ${weeksToPreview[0]}-${weeksToPreview[weeksToPreview.length - 1]}\n`
  );

  let lastReport = null;
  for (const displayWeek of weeksToPreview) {
    ...
    if (!playoffReport) {
      console.log(`Week ${displayWeek}: no playoff report content.\n`);
      continue;
    }

    console.log(playoffReport.textMessage);
    console.log("");
    lastReport = playoffReport;
  }

  if (!lastReport) {
    console.log("No weekly playoff report to preview yet.");
    return;
  }
  ...
  if (args.send) {
    ...
    await sendPreviewToChat({ chatId: targetChatId, message: lastReport.textMessage });
  }
}
...
async function sendPreviewToChat({ chatId, message }) {
  ...
  try {
    console.log("");
    console.log(`Sending preview weekly playoff report to test chat ${chatId}`);
    await startSnapchatSession(bot);
    await sendMessageWithRetry(bot, chatId, message);
    console.log("Preview weekly playoff report sent.");
  } finally {
    if (bot.browser) {
...
function clampWeek(week, minimumWeek, maximumWeek) {
  if (!Number.isFinite(week)) {
    return minimumWeek;
  }

  return Math.max(minimumWeek, Math.min(maximumWeek, Math.trunc(week)));
}
